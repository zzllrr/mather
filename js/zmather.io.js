...
,STRUC={
	...
	'根式':[$A(zlrA3("kroot('x'",['',',3',',4'],')')),
		["$kfrac([kroot(2),2])$",
			"φ=$kfrac([kroot(5)+'-1','2'])$",
			"x=$kfrac(['-b±'+kroot('b^2-4ac'),'2a'])$"
		],
		$A(zlrA3('mroots(',ZLR("[2,3],['','x','y'],'','','+-',1 [2,3,4,5],['',1,10,100,1000],'',1,'+',1"),",'')"))
	],
	...
	'左右括号':Arrf($A,[zlrA3("zp('x,y'",[
		...
		["pp('x,y')",
		"zp('x,y','','<','>')"],
		zlrA3('big(',ZLR('0 0,1 1 1,1 2 2,1 3 3,1'),')'),
		zlrA3('big(',ZLR('0,2 0,3 1,2 1,3 2,2 2,3 3,2 3,3'),')'),
		zlrA3('big(',ZLR('0,4 0,5 1,4 1,5 2,4 2,5 3,4 3,5'),')'),
	]),

	'矩阵':Arrf($A,[zlrA3("zmtrx([[1,2",[
			"],[3,4]]",
			"],[3,4]],'','r2c2'",
			",3,4],[5,6,7,8]],'','c2'"
		],')'),
	...
	'大型求和':[['\\sum']].concat(Arrf($A,[zlrA3("sum('i',0,'+','f',",seqA(0,4),",'')"),
		zlrA3("sum('i',0,'+','f',",seqA(4,3),",'')").concat(
		["Opr('','-','+','f','*')"]
		)
	])),

	'大型求积':[['\\prod']].concat(Arrf($A,[zlrA3("prod('i',0,'+','f',",seqA(0,5),",'')"),
	...
	'积分':[['\\int']].concat(Arrf($A,[zlrA3("intl('f','-','+','x',",seqA(0,4),",'')"),
	...
	'上':Arrf($A,[zlrA3("kxo('a','",'-→↔←I↼⇀'.split(''),"')"),//katex 暂不支持 ⇐ ⇒
		zlrA3("kxo('a','",'{(<~>='.split(''),"')")
	]),
	
	'下':Arrf($A,[zlrA3("kxu('a','",'-→↔←I'.split(''),"')"),//katex 暂不支持  ↼ ⇀ < > ⇐ ⇒ 
		zlrA3("kxu('a','",'{(~='.split(''),"')"),
	]),

	'上下备注':Arrf($A,[zlrA3("eq(1,'",'→↔←↩↪↞↠'.split(''),"',2)"),//katex 暂不支持 ⇆ ↤	//←↔→⇐⇔⇒=↩↪↞↠↼⇀↽⇁⇋⇌⇄↦
		zlrA3("eq(1,'",'↼⇀↽⇁↦'.split(''),"',2)"),

	]),

	'环绕备注':[['\\stackrel{1}{\\longrightarrow}','\\stackrel{1}{\\longleftarrow}'],
	...
	'二项式':Arrf($A,[zlrA3("binom('n+1','n'",[",'c'",'',",'d'",",'t'"],')'),
	]),
	
	'大型分式括号':Arrf($A,[zlrA3("genfrac(1,2",[
			",'','',1",
			",'','',1,0",
			",'','',1,1",
			",'','','',0",
