...
wiki['3n+1']=function(){
	...
	定理1：& T(x)是奇数且
	f(T(x))∈ B => T(x) ∈A \\\\&（根据集合A、B的定义易证）\\\\
	定理2：&
	x ∈ A => $piece([
	'g(x)，g^{(n)}(x) ∈ A（见证明(1)、(2)）',
	'f(x)，2^kx，2^kf(x)∈ B（可猜测B中有且只有这三种类型，类型可能有重叠）',
	'f(f(x))，f^{(n)}(x)∈ B（不易证，可能不恒成立）'
	])$ \\\\
	
	定理3：&
	x ∈ B => $piece([
	'f(x)，f^{(-1)}(x)（奇数时，易证），g^{(n)}(x) ∈ A?',
	'f(x)，2^kx（易证），2^kf(x)∈ B?'])$ \\\\

	定理4：&
	x ∈ A, y ∈ A => $piece([
	'f(A)+f(B) ∈ B ~当'+kmod('x','y≡1或-1',4)+'，且'+kfrac('x+y/2')+'∈ A时',
	''])$ \\\\
