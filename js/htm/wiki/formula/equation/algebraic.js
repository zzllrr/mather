...
wiki['Formula/Equation/Algebraic']=Kx(

detail('一些特殊的整系数代数方程',Table([ZLR('解形式 等价方程 简化方程 方程解')],[
	...
	['u+'+kroot('v',3)+' = x', ['$(x-u)^3 = v$',ksc('x^3-3ux^2+3u^2x-u^3-v=0')].join(hr),
		hrA(ksc(['x^3+3ax^2+3a^2x+b = 0','x^3+px^2+{p^2}\\/3x+q=0'])),
		['$x = -a+'+kroot('a^3-b',3)+'$',
	...
	['', ['$y^3+3py+2q = 0$','$y = x+3a$'].join(hr),
		...
		[[
		'判别式$Δ = q^2+p^3'+piece([['>0','有1个实根、2个共轭复根'],['=0','有3个实根（其中至少有2个相等）'],['<0','有3个不同实根']])+'$',
		
		'$'+Eq([['X','-a'+zmtrx(MfS('[1 1 1]\''))+' - '+zmtrx(MfS('[1 1;ω ω^2;ω^2 ω]'))+zmtrx([[kroot('q-'+kroot('q^2+p^3'),3)],[kroot('q+'+kroot('q^2+p^3'),3)]])],
			'-a'+zmtrx(MfS('[1 1 1]\''))+' + '+zmtrx(MfS('[1 1;ω ω^2;ω^2 ω]'))+zmtrx([[kroot('-q+'+kroot('q^2+p^3'),3)],[kroot('-q-'+kroot('q^2+p^3'),3)]]),
			'-a'+zmtrx(MfS('[1 1 1]\''))+' - '+zmtrx(MfS('[1 1;ω ω^2;ω^2 ω]'))+zmtrx([[kroot('q-'+kroot('Δ'),3)],[kroot('q+'+kroot('Δ'),3)]]),
			'-a'+zmtrx(MfS('[1 1 1]\''))+' + '+zmtrx(MfS('[1 1;ω ω^2;ω^2 ω]'))+zmtrx([[kroot('-q+'+kroot('Δ'),3)],[kroot('-q-'+kroot('Δ'),3)]]),

			'-a'+zmtrx(MfS('[1 1 1]\''))+' - '+zmtrx(MfS('[1 1;ω \\overline{ω};\\overline{ω} ω]'))+zmtrx([[kroot('q-'+kroot('Δ'),3)],[kroot('q+'+kroot('Δ'),3)]]),
			'-a'+zmtrx(MfS('[1 1 1]\''))+' + '+zmtrx(MfS('[1 1;ω \\overline{ω};\\overline{ω} ω]'))+zmtrx([[kroot('-q+'+kroot('Δ'),3)],[kroot('-q-'+kroot('Δ'),3)]])
		])+'$',
		...
		ul(['$当Δ=0时，x = '+piece([['-a - 2'+kroot('q',3),''],['-a + '+kroot('q',3),'两重']])+'$',
			...
			['$当Δ>0时, $','$ x = '+piece([
					...
					['-a - ω'+kroot('q+'+kroot('Δ'),3)+' - '+kxo('ω')+kroot('q-'+kroot('Δ'),3)],
					['-a - '+kxo('ω')+kroot('q+'+kroot('Δ'),3)+' - ω'+kroot('q-'+kroot('Δ'),3)],
					])+'$',
				...
				'$= '+piece([
					['-a - ξ'],
					['-a + '+frac('ξ',2,'')+' + '+frac(kroot(3)+'ηi',2,'')+' = -a - r \\cos '+zp('θ+'+frac('2π',3,'')) +'=-a-2'+kroot('|p|')+'\\cos '+zp('θ+'+frac('2π',3,''))],
					['-a + '+frac('ξ',2,'')+' - '+frac(kroot(3)+'ηi',2,'')+' = -a - r \\cos '+zp('θ-'+frac('2π',3,'')) +'=-a-2'+kroot('|p|')+'\\cos '+zp('θ+'+frac('2π',3,''))],
					])+'$',
				...
				'实数 $r = '+kroot('|ξ^2-η^2|')+' = 2'+kroot('|q^2-Δ|',6)+'=2'+kroot('|p|')+'$',
				'实数 $\\cosθ = '+frac('ξ','r','')+'$',
				'虚数 $\\sinθ = '+frac('ηi','r','')+'$'
			...
			['$当Δ < 0时,$','$ x = '+piece([
				...
				'$= '+piece([
					['-a - ξ'],
					['-a + '+frac('ξ',2,'')+' + '+frac(kroot(3)+'ηi',2,'')+' = -a - r \\cos '+lrp('','θ+'+frac('2π',3,''),'','')],
					['-a + '+frac('ξ',2,'')+' - '+frac(kroot(3)+'ηi',2,'')+' = -a - r \\cos '+lrp('','θ-'+frac('2π',3,''),'','')],
					])+'$',
				...
				'$r = '+kroot('ξ^2-η^2')+' = 2'+kroot('q^2-Δ',6)+'=2'+kroot('-p')+'$',
				'$\\cosθ = '+frac('ξ','r','')+'$',
				'$\\sinθ = '+frac('ηi','r','')+'$'
				].join(br),
