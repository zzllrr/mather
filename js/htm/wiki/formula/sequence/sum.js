...
detail('数列内积和（卷积和）公式',Table([ZLR('前n项求和 记法 结果')],[
	...
	[['(n-1)1^2+','(n-2)2^2+','⋯+','1(n-1)^2'].join(kbr),
		'从正方形n^2个格点中选4点，可以连成正方形的个数'+kbr+sum('i',1,'n-1','(n-i)i^2','','')+kbr+'='+
		sum('i',1,'n-1','i(n-i)^2','','')+kbr+'='+

		sum('i',1,'n-1',sum('j',1,'i','j^2','',''),'','')+kbr+'='+
		sum('i',1,'n-1','\\frac{i(i+1)(2i+1)}{6}','','')+kbr+'='+

		sum('i',1,'n-1','i^2','','')+zp('正方格=\\frac{n(n-1)(2n-1)}{6}=1\\/4C_{2n}^3')+kbr+'+'+
		sum('i',1,'[(n-1)/2]','(n-2i)^2','','')+zp('菱方格=\\frac{n(n-1)(n-2)}{6}=C_n^3')+kbr+'+'+
		sum('i',1,'n-3','i(i+1)(i+2)/3','','')+zp('斜方格=\\frac{n(n-1)(n-2)(n-3)}{12}=2C_n^4'),




		Eq([frac(1,12,'')+'n^2(n-1)(n+1)', frac(1,12,'')+'n^2(n^2-1)',
			'C_{n+1}^2C_{n}^2/3',
			sum('i',1,'n','i^3','','')+'-2n'+sum('i',1,'n','i^2','','')+'+n^2'+sum('i',1,'n','i','','')
		])
	],
