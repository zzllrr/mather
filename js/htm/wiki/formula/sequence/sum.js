...
detail('求和公式',Table([ZLR('前n项求和 记法 结果')],[
	...
	[ZLR3('',ZLR('1 2 3 ⋯ n'),'^m','+')+kbr+'等幂和',
		sum('i',1,'n','i^m','',''),
		Eq([frac(1,'m+1','')+'n^2(n+1)^2(2n^2+2n-1)',
			'1\\/{m+1}'+sum('j',0,'m',binom('m+1','j')+'B_jn^{m+1-j}','',''),
			'1\\/{m+1}'+sum('j',0,'m',binom('m+1','j')+'B^jn^{m+1-j}','','')+kxf(' 阴影形式 Umbral form'),
	...
	[['q+2⋅q^2+3⋅q^3+⋯+n⋅q^n'].join(kbr),
	sum('i',1,'n','i⋅q^i','',''),
	[
	frac(sum('i',1,'n','q^i','','')+'-n⋅q^{n+1}','1-q',''),
	'数列×q，错位相减'
	].join(kbr)
],
	[['aq+(a+d)⋅q^2+(a+2d)⋅q^3','+⋯+(a+(n-1)d)⋅q^n'].join(kbr),
	sum('i',1,'n','(a+(i-1)d)⋅q^i','','')+kbr+'='+sum('i',1,'n','a_i⋅q^i','',''),
	[
	frac('d'+sum('i',1,'n','q^i','','')+'+a_0q-a_n⋅q^{n+1}','1-q',''),
	'数列×q，错位相减'
	].join(kbr)
],
]
